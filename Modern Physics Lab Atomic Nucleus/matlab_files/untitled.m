function functionForPlastic(thickness,densityPlastic,expDataPlastic)
% FUNCTIONFORPLASTIC do everything like alumMatlabAnalysis for plastic only
%	OUT = FUNCTIONFORPLASTIC(IN)
%
% Long description
...
y.plasOne=expDataPlastic.data(:,2)'; % Energy in keV
x.plasOne=expDataPlastic.data(:,1)'; % plate numbers
sigma.plasOne = expDataPlastic.data(:,3)';
...
% stop the electrons
zero.thicknessPlas = zero.xPlas * thickness;
% Define R(E) = kE^B
range.plasticmeters = zero.thicknessPlas - x.plasOne * thickness; % in meters
range.plasticcentimeters = range.plasticmeters * 100; % in cm
range.plasticgcm2 = range.plasticcentimeters * densityPlastic; % in g/cm^2
