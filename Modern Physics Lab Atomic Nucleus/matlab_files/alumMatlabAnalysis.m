...
for ii=1:15
    ...
    %% Print out latest statistics
    fprintf('\n')
    fprintf('Final Fit Parameters %g\n',ii)
    fprintf('Aluminum:\n');
    fprintf(' k = %g +/- %g \n',k.aluminum,k.aluminum*dc.aluminum);
    fprintf(' B = %g +/- %g \n',B.aluminum,dB.aluminum);
    fprintf(' Chi-squared: %g\n',chisqr.two);
    ...
    fprintf(' Chi-squared: %g\n',chisqr.plasTwo);
    fprintf('         N-M: %g \n',n.plastic-m.plastic);
    chisqr.allsaved(1,ii+1) = chisqr.plasTwo;
    chisqr.allsaved(2,ii+1) = chisqr.two;
end
%% Plot
figure('Units', 'pixels', ...
    'Position', [100 100 500 375]);
plotOf.OurErrorBarsPlastic = errorbar(energy.plasMeV,...
    exp(yy.plasTwo),dR.plastic,'.'...
...
%% Print out latest statistics
fprintf('\n')
fprintf('Final Fit Parameters last\n')
fprintf('Aluminum:\n');
fprintf(' k = %g +/- %g \n',k.aluminum,k.aluminum*dc.aluminum);
fprintf(' B = %g +/- %g \n',B.aluminum,dB.aluminum);
fprintf(' Chi-squared: %g\n',chisqr.two);
...
makeTableLatex( dR.plastic,sigma.plasOne',...
    dy.plasRange',...
    dR.aluminum,sigma.one',...
    dy.alumRange',...
    [figurePath 'firstTableTry.tex'] );
